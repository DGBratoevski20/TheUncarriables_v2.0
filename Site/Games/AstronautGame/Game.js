  ...
  if (CheckArrowUp == 1) {
    ...
    setInterval(function () {
      ...
      FirstVarForTimeOut = setTimeout(function () {
        ...
        document.querySelector("#point").style.left =
          Math.round(Math.random() * 80 + 1) + "%";
    ...
    setInterval(function () {
      ...
      SecondVarForTimeOut = setTimeout(function () {
        ...
        document.querySelector("#bomb").style.left =
    ...
    setInterval(function () {
      document.querySelector("#bomb2").style.animation =
        "clouds 1500ms infinite";

      ThirdVarForTimeOut = setTimeout(function () {
        document.querySelector("#bomb2").style.left =
    ...
    SecondVarForInterval = setInterval(function () {
      ...
      if (counterForMetres >= 12000 && counterForMetres < 50000) {
        ...
        document.querySelector(".area").style.backgroundColor =
          "rgb(102, 102, 255)";

        document.querySelector("#cloud").style.animation = "none";
        document.querySelector("#cloud").style.animation =
          "left-right 6s infinite";

        document.querySelector("#cloud2").style.animation = "none";
        document.querySelector("#cloud2").style.animation =
          "right-left 8s infinite";

        document.querySelector("#cloud").style.backgroundImage =
          "url('images/alient.png')";
        document.querySelector("#cloud2").style.backgroundImage =
          "url('images/aircraft2.png')";
        ...
        document.querySelector("#cloud").style.animation = "none";
        document.querySelector("#cloud").style.animation =
          "left-right 9s infinite";

        document.querySelector("#cloud2").style.animation = "none";
        document.querySelector("#cloud2").style.animation =
          "right-left 10s infinite";
        ...
        document.querySelector("#cloud").style.animation = "none";
        document.querySelector("#cloud").style.animation =
          "left-right 10s infinite";

        document.querySelector("#cloud2").style.animation = "none";
        document.querySelector("#cloud2").style.animation =
          "right-left 12s infinite";

        document.querySelector("#cloud").style.backgroundImage =
          "url('images/alient.png')";
        document.querySelector("#cloud2").style.backgroundImage =
          "url('images/comet.png')";
      }
    }, 400);
  }
...
setInterval(function () {
  ...
  function Bomb1() {
    if (
      Math.round(firstBomb.left) - Math.round(hero.left) >= -40 &&
      Math.round(firstBomb.left) - Math.round(hero.left) <= 60 &&
      Math.round(firstBomb.top) - Math.round(hero.top) >= -40 &&
      Math.round(firstBomb.top) < Math.round(hero.top) + 60
    ) {
      return true;
    }
  }
  function Bomb2() {
    if (
      Math.round(secondBomb.left) - Math.round(hero.left) >= -40 &&
      Math.round(secondBomb.left) - Math.round(hero.left) <= 60 &&
      Math.round(secondBomb.top) - Math.round(hero.top) >= -40 &&
      Math.round(secondBomb.top) < Math.round(hero.top) + 60
    ) {
      return true;
    }
  }
  if (
    Math.round(point.left) - Math.round(hero.left) >= -40 &&
    Math.round(point.left) - Math.round(hero.left) <= 60 &&
    Math.round(point.top) - Math.round(hero.top) >= -40 &&
    Math.round(point.top) < Math.round(hero.top) + 50
  ) {
    document.querySelector("#point").style.animation = "none";
...
window.addEventListener("touchstart", function (TouchEvent) {
  ...
  if (
    XOftouching >= Math.round(ButtonLeft.left) &&
    XOftouching <= Math.round(ButtonLeft.left) + ButtonLeft.width &&
    YOftouching >= Math.round(ButtonLeft.top) &&
    YOftouching <= Math.round(ButtonLeft.top) + ButtonLeft.height
  ) {
    if (left < 15) {
      helperOnLeftMob = 0;
    } else {
    ...
    XOftouching >= Math.round(ButtonRight.left) &&
    XOftouching <= Math.round(ButtonRight.left) + ButtonRight.width &&
    YOftouching >= Math.round(ButtonRight.top) &&
    YOftouching <= Math.round(ButtonRight.top) + ButtonRight.height
  ) {
    if (left > 76) {
      helperOnLeftMob = 0;
    } else {
    ...
    YOftouching >= Math.round(buttonStartGame.top) &&
    YOftouching <= Math.round(buttonStartGame.top) + buttonStartGame.height &&
    XOftouching >= Math.round(buttonStartGame.left) &&
    XOftouching <= Math.round(buttonStartGame.left) + buttonStartGame.width
  ) {
    CheckArrowUp++;
    UP();
  }
...
FirstvarForInterval = setInterval(function () {
  if (fuel <= 0) {
    ...
    if (document.body.clientWidth < 2060 && document.body.clientWidth > 400) {
      document.querySelector(".info").querySelector("h1").style.fontSize =
        "1.3rem";
    }
